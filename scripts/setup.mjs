...
async function main() {
  ...
  note("Free local model. Install at https://ollama.com if you don't have it.");
  const OLLAMA_BASE_URL = await ask("Ollama base URL", {
    default: current.OLLAMA_BASE_URL || "http://localhost:11434",
  });

  // ── Gemini ──
  header("Gemini (cloud AI, optional)");
  note("Optional — Ollama works fine on its own.");
  note("Free key at: https://aistudio.google.com/app/apikey");
  const useGemini = await confirm("Add a Gemini API key?", !!current.GEMINI_API_KEY);
  const GEMINI_API_KEY = useGemini
    ? await ask("Gemini API Key", { default: current.GEMINI_API_KEY })
    : "";
...
# ── Gemini ──
GEMINI_API_KEY=${quote(GEMINI_API_KEY)}
